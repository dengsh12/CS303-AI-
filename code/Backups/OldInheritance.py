...
class AI(object) :
    ...
    def __init__(self, chessboard_size, color, time_out) :
        ...
        self.c1 = 1
        self.c2 = 0
        self.c3 = [0 for i in range(2)]
        self.c4 = [0 for i in range(2)]
        self.color = color
    ...
    def get_mark(self, chessboard: list, nowTurn) :
        ...
        ans = self.c1 * initial + self.c2 * stable+arb+self.c4[k]*front
        return ans
...
def product(waiters: list) :
    ...
    for i in range(l) :
        ...
        son.c2 = 0
        son.c3 = [1 for i in range(2)]
        son.c4= [1 for i in range(2)]
        fu = waiter.c2
        mu = mate.c2
        son.c2 = get_subv(fu, mu, 8)
        for i in range(2):
            fu=-waiter.c3[i]
            mu=-mate.c3[i]
            son.c3[i]=-get_subv(fu,mu,7)
            fu=-waiter.c4[i]
            mu=-mate.c4[i]
...
def all_playto(waiter: list, normal) :
    ans = [0 for i in range(len(waiter))]
    for idx, wt in enumerate(waiter) :
        wt.color = COLOR_BLACK
        normal.color = COLOR_WHITE
        res = playto(wt, normal)
        ans[idx] += res
        wt.color = COLOR_WHITE
        normal.color = COLOR_WHITE
        res = playto(normal, waiter)
        ans[idx] -= res
    return ans


def all_playto_muti(lis: list) :
    normal = lis[len(lis) - 1]
    waiter = lis[0 :len(lis) - 1]
    return all_playto(waiter, normal)
...
def inheritance(waiters: list) :
    ...
    if True:
        ...
        for i in range(int(waiter_num/2)) :
            ...
            out.writelines(['life time:'+str(wt.live_time),'black_wins:' + str(wt.black_win) + ',white_wins:' + str(
                wt.white_win) + ',total_win:' + str(wt.win_num) + ',ties:' + str(wt.tie_num) + "\n",
    ...
    winner: AI = waiters[0]
    print(f"life time:{winner.live_time},black_win:{winner.black_win},white_win:{winner.white_win},total_win:{winner.win_num},ties:{winner.tie_num}")
...
if __name__ == "__main__" :
    ...
    basicab.c1 = 1
    basicab.c2 = 0
    basicab.c3 = [0 for i in range(2)]
    basicab.c4 = [0 for i in range(2)]
    waiters = []
    for i in range(waiter_num) :
        here = AI(8, 0, 100)
        waiters.append(here)
        here.c2 = random.randint(0,255)
        for j in range(2):
            here.c3[j]=random.randint(-127,0)
